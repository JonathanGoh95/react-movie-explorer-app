    ...
    return(
        <>
            <form className='flex gap-10 justify-center' onSubmit={handleSubmit}>
                <input className='border-2 text-3xl rounded-lg w-1/2 text-center' value={query} type='text' placeholder='Type the Movie Title here...' onChange={({target})=>setQuery(target.value)}></input>
                <input className='border-2 text-3xl rounded-lg w-1/7 text-center' value={movieYear} type='number' placeholder='Filter by Release Year...' onChange={({target})=>setMovieYear(target.value)} disabled={query === ''}></input>
